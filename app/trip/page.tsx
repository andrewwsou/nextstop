...
function TripForm() {
  ...
  const handleUseCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => { const { latitude, longitude } = position.coords; setStart(`${latitude}, ${longitude}`); },
  ...
  const handlePlanTrip = () => {
    ...
    const allowedTransit = Object.entries(transitTypes).filter(([, isEnabled]) => isEnabled).map(([type]) => type).join(",");
    if (!trimmedStart || !trimmedDestination) { setFormError("Enter both a start and destination."); return; }
    if (!allowedTransit) { setFormError("Choose at least one transit type."); return; }
    setFormError("");
    router.push(`/trip/results?start=${encodeURIComponent(trimmedStart)}&destination=${encodeURIComponent(trimmedDestination)}&transit=${encodeURIComponent(allowedTransit)}&date=${encodeURIComponent(tripDate)}&time=${encodeURIComponent(tripTime)}`);
  };
    ...
    <main className="min-h-screen bg-[#0b0c0e] text-[#f3f4f6] px-6 py-16 max-w-2xl mx-auto" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className="space-y-8">
        ...
        <section aria-labelledby="locations-heading" className="space-y-6">
          ...
          <h2 id="locations-heading" className="text-xs font-bold tracking-widest text-[#848d9a] uppercase">
            Trip Locations
          </h2>

          <div>
            <div className="mb-2.5 flex items-center justify-between">
              <label htmlFor="start-input" className="text-xs font-bold tracking-widest text-[#848d9a] uppercase">
                Starting Point
              </label>
              <button
            ...
            <input
              ...
              className="w-full rounded-none border-b border-[#222630] bg-transparent py-3 text-sm text-[#f3f4f6] placeholder-[#848d9a] outline-none transition-colors focus:border-[#3ecfb2]"
            />
          ...
          <div>
            <label htmlFor="destination-input" className="mb-2.5 block text-xs font-bold tracking-widest text-[#848d9a] uppercase">
              Destination
            </label>
            <input
        ...
        <section aria-labelledby="datetime-heading" className="pt-2">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <label htmlFor="trip-date" className="mb-2.5 block text-xs font-semibold tracking-widest text-[#848d9a] uppercase">
                Date
              </label>
              <input
                id="trip-date"
            ...
            <div>
              <label htmlFor="trip-time" className="mb-2.5 block text-xs font-semibold tracking-widest text-[#848d9a] uppercase">
                Time
              </label>
              <input
                id="trip-time"
